...
2. **发布页异常**：所有设置都正常渲染，唯独 `<xhs-publish-btn>` 组件**对 CDP 不可见**
3. **静默失败**：不报错、不弹验证码，只是「发布」按钮不存在于 CDP 的 DOM 中
4. **不是被删除**：MutationObserver 监控显示按钮从未被创建。Vue 组件初始化时检测到 CDP，`mounted()` 直接跳过渲染
5. **🔑 关键发现：按钮在 VNC 上可见！** — CDP 的 `Runtime.evaluate` 搜不到「发布」，但在 VNC 上肉眼能看到按钮。CDP 搜不到 ≠ 按钮不存在。风控只在 CDP 的 DOM 视图里移除了按钮，Vue 的视觉渲染层仍然绘制了它
...
4. **xdotool 盲扫不可靠**——按钮位置随页面内容变化，36 个位置全空。**但如果用截图识别按钮位置，就能精准点击**——因为按钮在 VNC 上确实可见
5. **CDP 搜不到 ≠ 按钮不存在**——这是本次最大的误判。VNC 上肉眼可见按钮，CDP 却报「不存在」，说明风控只在 DevTools 的 DOM 视图层做了手脚
6. **保持 `--remote-debugging-port=19922`**——这是当前最佳伪装方案，Pipe 模式已废弃
